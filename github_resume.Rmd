...
subtitle: "Veni-funded developmental data scientist, Open Science faculty ambassador, statistical consulting editor. Using machine learning to predict teenagers' emotional problems"
date: "`r Sys.Date()`"
...
#pub$title[grepl(":", pub$title)] <- gsub("(?<!orner):.+$", "", pub$title[grepl(":", pub$title)], perl = TRUE)
pub$title[nchar(pub$title) > 150] <- paste0(substr(pub$title[nchar(pub$title) > 150], 1, 149), "…")
  #gsub("^.+orner: ", "", pub$title)
...
pub <- pub[!pub$Publication %in% c("", "<i>Onderwijskunde UU</i>"), ]
rownames(pub) <- NULL
